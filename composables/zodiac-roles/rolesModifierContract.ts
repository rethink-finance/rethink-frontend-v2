import { BigNumberish, BytesLike, ethers, ContractTransaction } from "ethers"
...
function getTargetTransaction(contract: Roles, role: RoleContextState, target: Target): Promise<ContractTransaction> {
  switch (target.type) {
...
export const updateRole = async (
  ...
  const txs = [...role.targets.list, ...role.targets.add].map(async (target) => {
    ...
    try {
      ...
      const targetABI = targetAbis[target.address];
      const contract = new Interface(targetABI);

      functions = getWriteFunctions(contract)
        .reduce((obj, func) => {
    ...
    const targetLevelTxs: Promise<ContractTransaction>[] = updateEvents
    ...
    const updateFunctionOptionTxs: Promise<ContractTransaction>[] = updateEvents
      .filter(
    ...
    const functionLevelTxs: Promise<ContractTransaction>[] = updateEvents
      .filter((event) => event.level === Level.SCOPE_FUNCTION && event.targetAddress === target.address)
      .map((event): Promise<ContractTransaction> => {
        const value = event.value as FunctionCondition
    ...
    const paramLevelTxs: Promise<ContractTransaction>[] = Object.entries(paramEventsPerFunction)
      .map(([sighash, params]) => {
...
export const executeTxsGnosisSafe = async (txs: ContractTransaction[]) => {
  const safeSDK = new SafeAppsSDK()
  const { safeTxHash } = await safeSDK.txs.send({ txs: txs.map(convertTxToSafeTx) })
  return safeTxHash
}

export const executeTxsInjectedProvider = async (txs: ContractTransaction[]) =>
  await Promise.all(
    txs.map(async (tx) => {
...
function convertTxToSafeTx(tx: ContractTransaction): BaseTransaction {
  return {
    to: tx.to as string,
    value: "0",
