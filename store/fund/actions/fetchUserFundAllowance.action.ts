...
export const fetchUserFundAllowanceAction = async (): Promise<any> => {
  ...
  fundStore.userBalances.fundAllowance = BigInt("0");
  ...
  fundStore.userBalances.fundAllowance = await fundStore.callWithRetry(() =>
  ...
  console.log(
    `user fund allowance of ${fundStore.fund?.baseToken?.symbol} is ${fundStore.userBalances.fundAllowance}`,
  );
  return fundStore.userBalances.fundAllowance;
};
