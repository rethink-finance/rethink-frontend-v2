...
const state = {
  ...
  contract: null,
  decimals: 18,
  lpAllowance: 0, // liquidity pool contract Matic allowance for current user
...
const getters = {
  ...
  getMaticContract(state) {
    return state.contract;
  },
  getFundMaticAllowance(state) {
    return state.lpAllowance;
  },
  getUserMaticBalance(state) {
    return state.userBalance;
  },
  usesPermitMatic(state) {
    return state.permit;
  },
};
