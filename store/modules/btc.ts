...
const state = {
  ...
  contract: null,
  decimals: 18,
  lpAllowance: 0, // liquidity pool contract Btc allowance for current user
...
const getters = {
  ...
  getBtcContract(state) {
    return state.contract;
  },
  getFundBtcAllowance(state) {
    return state.lpAllowance;
  },
  getUserBtcBalance(state) {
