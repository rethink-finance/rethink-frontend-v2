  ...
  actions: {
          ...
          try {
            ...
            if (fund) {
              ...
              const cumulativeReturnPercent = fundLastNavUpdateExists ?
                calculateCumulativeReturnPercent(fund.totalDepositBalance, fund.totalNAVWei, baseTokenDecimals) :
                0;

              fund.totalNAVWei = fundLastNavUpdateExists ? fund.totalNAVWei : fund.totalDepositBalance;
    ...
    async fetchAllNavMethods(fundsInfoArrays: any[]) {
      ...
      for (const [fundIndex, fundNavData] of allFundsNavData.entries()) {
        ...
        this.fundNAVUpdates[fundAddress] = await this.fundStore.parseFundNAVUpdates(
          allFundsNavData[fundIndex],
          fundAddress, fundContract,
        );
