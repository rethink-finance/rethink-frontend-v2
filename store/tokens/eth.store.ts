...
export const useWethStore = defineStore("weth", {
  state: (): IState => ({
    abi: null,
    address: null,
    contract: null,
  ...
  getters: {
    getEthDecimals: (state) => state.decimals,
    getEthAbi: (state) => state.abi,
    getEthAddress: (state) => state.address,
    getEthContract: (state) => state.contract,
    getFundEthAllowance: (state) => state.fundAllowance,
    getUserEthBalance: (state) => state.userBalance,
    usesPermitEth: (state) => state.permit,
  },
  actions: {
    fetchContract() {
