...
import { useExplorerTransactionsFetcher } from "~/composables/transactionsFetcher/useExplorerTransactionsFetcher";
...
// Local state
const delegatedEntry = ref<Entry[]>(
...
const fetchBlocksInBatch = (blockNumbers: number[]): Promise<any[]> => {
  ...
  return web3Store.callWithRetry(
    props.chainId,
    () => {
...
const { loading, error, fetchTransactions, fetchTokenTransfers } =
...
const getTransactionCallData = async (
  ...
  const totalBlocks = blockRanges.reduce(
    (sum, range) => sum + (range.endBlock - range.startBlock + 1),
    0,
  );
  ...
  for (const range of blockRanges) {
    const blockNumbers = Array.from(
      { length: range.endBlock - range.startBlock + 1 },
      (_, i) => range.startBlock + i,
    );
    for (let i = 0; i < blockNumbers.length; i += batchSize) {
      ...
      const batchTransactions = getTransactionsForAddress(
        address,
        batchResponse,
      );
      ...
      parsingProgress.value =
        totalBlocks > 0 ? (processedBlocks / totalBlocks) * 100 : 0;
    }
  }
...
const createTokenApprovalMap = (
  filtApprovals: string[],
): Map<string, string[]> => {
...
const generateSimplePermissions = (
  ...
  for (const p of processedData) {
    if (p.is_approval) {
      ...
      const tokenFound = p.words
        .map((word, i) =>
          uniqueApprovals.some((addr) =>
            word.includes(addr.slice(2).toLowerCase()),
          )
            ? i
            : -1,
        )
  ...
  const zeroPaddedTokenList = uniqueApprovals.map(
  ...
  const approvalPermissions =
    generateTokenApprovalPermissions(tokenApprovalsMap);
  permissions.push(
    ...approvalPermissions.map((perm) => ({ ...perm, idx: permsIdx++ })),
  );
  ...
  for (const p of processedData) {
    ...
    if (ioiLen === 0 && tidxLen === 0) {
      const perms = JSON.parse(
        JSON.stringify(sampleScopeAllowFunction),
      ) as Permission;
      ...
      if (tidxLen !== 0) {
        if (tidxLen === 1) {
          if (uniqueApprovals.length === 1) {
            const perms = JSON.parse(
              JSON.stringify(sampleScopeParameter),
            ) as Permission;
            perms.value[1].data = p.contract;
            perms.value[2].data = p.func_sig;
            ...
            const perms = JSON.parse(
              JSON.stringify(sampleScopeParameterAsOneOf),
            ) as Permission;
            perms.value[1].data = p.contract;
            perms.value[2].data = p.func_sig;
            perms.value[3].data = String(p.token_found_idx[0]);
      ...
      if (ioiLen !== 0) {
        for (const tIdx of p.indices_of_addr) {
          ...
          perms.value[5].data = "0";
          perms.value[6].data =
            "0x000000000000000000000000" + custodyAddress.slice(2);
          perms.idx = permsIdx++;
          permissions.push(perms);
        }
      }
...
const fetchAndGeneratePermissions = async () => {
  try {
    ...
    try {
      ...
      transactions = await getExplorerTransactions(
        addressInput.value,
        blockRanges,
      );
    } catch (e) {
      ...
      transactions = await getTransactionCallData(
        addressInput.value,
        blockRanges,
      );
    }

    permissions.value = generateSimplePermissions(
      transactions,
      props.safeAddress,
      addressInput.value,
    );
    ...
    toastStore.errorToast(
      "Failed to generate permissions: " + (err as Error).message,
    );
  } finally {
...
const updateTransactionsJsonField = () => {
  ...
  transactions.forEach((trx: any) => {
    ...
    detailsStep.steps[0].transactionsOverview = JSON.stringify(
      permissionsProposalStore.rawTransactions.map((trx: IRawTrx) => [
        trx.funcName,
        trx.args,
      ]),
      null,
    ...
    detailsStep.steps[0].transactionsRawJSON =
      permissionsProposalStore.rawTransactionsJson;
  });
};
...
const fieldsChanged = (stepName: string, subStepIndex: number, step: any) => {
  ...
  const currentInputs =
    delegatedEntry?.value?.[mainStepIndex]?.steps?.[subStepIndex];
  if (!currentInputs) {
